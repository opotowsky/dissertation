...
covered in Section \ref{sec:training}.
...
\ref{sec:statmodel} are three methods: \textit{k}-Nearest Neighbors, Decision
...
error or prediction error. This is outlined in Section \ref{sec:eval}. 
...
used, and model perfomance are in Section \ref{sec:mlback}.  Lastly, a review
of statistical methods being used in studies of forensics analysis is covered
in Section \ref{sec:stats4nf}. 

After the existing work is discussed, the methodology and implementation of the
experimental components is introduced next in Chapter \ref{ch:method}.  This
will cover the simulated training data in Sections \ref{sec:training} and
\ref{sec:inforeduc}, the the details for training models in Section
\ref{sec:statmodel}, and the process of model evaluation in Section
\ref{sec:eval}.

\todo[inline]{update with results sections} 
