...
\gls{BWR}s: 0.4\% for \textit{k}-nearest neighbors and decision trees, and
1.6\% for \gls{MLL} calculations. However, there are also some \gls{BWR}s that
are misclassified as \gls{PWR}s: 0.1\% for \textit{k}-nearest neighbors and
...
\begin{figure}[!htb]
  ...
  \caption[Prediction performance of enrichment regression with increasing 
          ...
          {Prediction performance of enrichment as measured by relative and 
           ...
           nuclide mass measurements in the training set.}
  \label{fig:randenri}
  \todo[inline]{double check these figures aren't orphaning any text}
\end{figure}  
...
\begin{figure}[!htb]
  ...
  \label{fig:randcool}
  \todo[inline]{swap inset with legend}
\end{figure}
