...
also gets perturbed by $\pm10\%$, \todo[inline]{Paul clarify? BL: But it will
be calculating the perturbation by using values of enrichment a full percentage
point higher? (I dont think this is a major issue btw, what you are doing
overall sounds fine)} and not more because the cross-section libraries in
...
\ref{tbl:rxtrparam}.  Additionally, natural variations in ${}^{234}\text{U}$
and ${}^{236}\text{U}$ were not considered.  The percentage of the two uranium
isotopes in fresh fuel were kept the same for all \gls{SNF} simulations:
0.0356\% for ${}^{234}\text{U}$ and 0.0184\% for ${}^{236}\text{U}$.  The
0-valued burnup and cooling time entries were then filtered out so that the
future calculations of relative error would be possible for all predictions.  
