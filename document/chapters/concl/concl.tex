...
\todo[inline]{Paul, is this OK to not address?  BL: I think that co-detection
of enrichment and burnup typically requires a neutron count as well as a gamma
scan so it is unsurprising that gamma alone is struggling.}
...
%There are also more advanced detection techniques being studied that could
%clarify peaks from nuclides that typically are not detectable. A Compton
%supression measurement technique allows low-intensity peaks to be more
%detectable \cite{compton_supp}. An Ultra-High Rate \gls{HPGe} system operates
%such that peaks at the high energy end of the spectrum can be detected at large
%enough counts \cite{snf_gamma}. It is possible these approaches could be
%simulated and studied at a statistical level. 
