%\documentclass[11pt,handout]{beamer}
\documentclass[10pt]{beamer}
\usetheme[white]{Wisconsin}
%\title[short title]{long title}
\title[Information Reduction Impacts on SNF Prediction]{Information Reduction Impacts on SNF Prediction for Nuclear Forensics}
%\subtitle[short subtitle]{long subtitle}
\subtitle[Research Overview \& Progress Check-in]{Research Overview \& Progress Check-in}
%\author[short name]{long name}
\author[A. Opotowsky]{Arrielle Opotowsky}
%\date[short date]{long date}
\date[07.06.2021]{7 June 2021}
%\institution[short name]{long name}
\institute[UW-Madison]{University of Wisconsin-Madison}

\usepackage{caption}
\usepackage{subcaption}
\usepackage{hyperref}
\usepackage{enumerate}
\usepackage{amsmath}
\usepackage{multicol}
\usepackage{lmodern}
\usepackage{mathtools}
\usepackage{changepage}
\setbeamertemplate{footline}[page number]
\setbeamertemplate{caption}[numbered]
\setbeamertemplate{bibliography item}[text]
\setbeamercolor{page number in head/foot}{fg=gray}

% thanks to katy huff: get rid of header on title page\dots
\makeatletter
    \newenvironment{withoutheadline}{
       \setbeamertemplate{headline}[default]
       \def\beamer@entrycode{\vspace*{-\headheight}}
    }{}
\makeatother

% highlight words
\setbeamercolor{alerted text}{fg=black,bg=lightgray}
\newcommand{\boxalert}[1]{{%
  \usebeamercolor{alerted text}\colorbox{bg}{\alert{#1}}%
}}

\begin{document}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% From uw-beamer Here's a handy bit of code to place at 
%% the beginning of your presentation (after \begin{document}):
\newcommand*{\alphabet}{ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz}
\newlength{\highlightheight}
\newlength{\highlightdepth}
\newlength{\highlightmargin}
\setlength{\highlightmargin}{2pt}
\settoheight{\highlightheight}{\alphabet}
\settodepth{\highlightdepth}{\alphabet}
\addtolength{\highlightheight}{\highlightmargin}
\addtolength{\highlightdepth}{\highlightmargin}
\addtolength{\highlightheight}{\highlightdepth}
\newcommand*{\Highlight}{\rlap{\textcolor{HighlightBackground}{\rule[-\highlightdepth]{\linewidth}{\highlightheight}}}}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%--------------------------------%%
\begin{withoutheadline}
\frame{\titlepage}
\end{withoutheadline}

%%--------------------------------%%
%\AtBeginSection[]{
%\begin{frame}
%  \frametitle{Outline}
%  \begin{multicols}{2}
%    \tableofcontents[currentsection]
%  \end{multicols}
%\end{frame}
%}

%\begin{frame}
%  \frametitle{X}
%  \begin{figure}[h!]
%    \centering
%    \includegraphics[width=0.9\textwidth]{.png}
%    \caption{x}
%  \end{figure}
%\end{frame}

\section{Introduction}
\include{intro}

\section{Methodology}
\subsection{Computational Approach}
\include{methodology}
\subsection{Reactor Database}
\include{training}
\subsection{Statistical Methods}
\include{algs}
\subsection{Performance Evaluation}
\include{eval}

\section{Results}
\subsection{Direct Isotopics}
\include{results1}
\subsection{Gamma Spectra}
\include{results2}

\section{Conclusions}
\input{summary}
\input{acks}

%
%\section{Literature Review}
%\subsection{Nuclear Forensics}
%\input{forensics}
%\subsection{Statistical Models}
%\input{statmods}
%\subsubsection{Algorithms for Prediction}
%\input{algs}
%\subsubsection{ML Model Assessment}
%\input{errors}
%\subsubsection{ML Model Validation}
%\input{optvalid}
%\subsection{Computational Tools}
%\input{comptools}
%\subsection{Previous Work}
%\input{prevwk}
%
%\section{Demonstration}
%\input{demometh}
%\subsection{Training Data}
%\input{training}
%\subsection{Reactor Parameter Prediction}
%\input{demoresults}
%\subsection{ML Model Validation}
%\input{demovalid}
%
%\section{Research Proposal}
%\include{propprep}
%\subsection{Experiment 1}
%\include{exp1}
%\subsection{Experiment 2}
%\include{exp2}
%\subsection{Experiment 3}
%\include{exp3}
%\subsection{Method Comparison}
%\include{methcompare}
%

%%--------------------------------%%
%%--------------------------------%%

\begin{frame}[allowframebreaks]
  \frametitle{References}
  \bibliographystyle{plain}
  {\footnotesize \bibliography{pres} }
\end{frame}

%%--------------------------------%%

\end{document}


